...
static uint64_t rehash16_mul(void *key, const uint64_t seed, const int i) {
...
static uint64_t hash16_mul(void *key, const int i) {
	return rehash16_mul(key, 14695981039346656037LLU, i);
}

static uint64_t hash(void *key, const int i) {
...
uint64_t htable_find_or_put(bddnode_t *key) {
	...
	const uint64_t attempts = 512;

	// calculate base hash
	const uint64_t _h = hash(key, 1);

	// copy the given bdd node to the data table
	//memcpy(data_p, key, sizeof(bddnode_t));
	upc_memput(&data[data_i], key, sizeof(bddnode_t));
	...
	int i; for (i = 0; i < attempts; i++) {
		...
		const uint64_t h = i == 0 ? STORAGE_ADDR(_h) : STORAGE_ADDR(hash(key, i + 1));
...
size_t htable_owner(bddnode_t *key) {
	// calculate the hash..
	const uint64_t _h = hash(key, 1);
	const uint64_t h = STORAGE_ADDR(_h);
...
void htable_set_data(uint64_t index, bddnode_t *node) {
	upc_memput(&data[index], node, sizeof(bddnode_t));
}
